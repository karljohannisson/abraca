...
1. Run `PYTHONPATH=.nose python3 -m maintainability`. A nonzero exit is not claimable. Hunt extras; delete if any slipped through. Claimable: verifies pass, no dormant/frozen code, no leftover hunt hits, headline \(W\) understood vs baseline ([metric.md](../metric.md) “What \(W\) can claim”).
