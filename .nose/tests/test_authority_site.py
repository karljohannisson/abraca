...
class AuthoritySiteTests(unittest.TestCase):
    def _sites(self, axis_id: str, headline: bool = True):
        ...
        ax = next(a for a in reg.axes if a.id == axis_id)
        return sites_for_axis(ax, reg, corpus, headline)
    ...
    def test_authority_site_scores_the_function_not_the_file(self) -> None:
        reg = load_registry(FIX)
        corpus = build_corpus(FIX, reg.code_roots, reg.exclude)
        ax = next(a for a in reg.axes if a.id == "report-formats")
        site = sites_for_axis(ax, reg, corpus, True)[0]
    ...
    def test_authority_site_falls_back_to_module_when_symbol_missing(self) -> None:
